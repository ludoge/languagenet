...
def plotstft(audiopath, binsize=2 ** 10, plotpath=None, colormap="gray", channel=0, name='tmp.png', alpha=1, offset=0):
    ...
    ims = ims[0:128, offset:offset + 128]
...
processed = os.listdir('spectrograms_downsampled')
processed = [x[:-4] for x in processed if '.png' in x]
...
for iter, line in enumerate(
    ...
    if filename not in processed:
        wavfile = 'tmp.wav'
        os.system('mpg123 -w ' + wavfile + ' trainingdata/' + filepath)
        """
        ...
        plotstft(wavfile, channel=0, name='spectrograms_downsampled/' + filename + '.png', alpha=1.0)
        os.remove(wavfile)
